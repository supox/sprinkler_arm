...
class LedButtonListener : public IInputListener, private ITimeListener
{
public:
	...
	virtual void OnInputChanged(unsigned int ButtonIndex, const bool ButtonValue)
	{
		if(m_bRunning)
			return;
		
		if(ButtonIndex != m_ButtonIndex || ButtonValue)
			return;
